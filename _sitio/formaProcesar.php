...
$elTokenServidor= $_SESSION[$nombreToquen];
 
 $ctrlTmp=encripta('decrypt',$ctrlTmp);
 $hanPasado=$hoySt-$ctrlTmp;
...
//echo"$ses $elTokenServidor ***";

	if($ses!=$elTokenServidor || $elTokenServidor=="" || $ses==""  || $ctrl!=""){
	die();	
	}
...
$datin=base64_decode($datin);	
	...
	$ext = getExtension($name);
 	$ext = strtolower($ext);
		...
		set_time_limit(0);
		$urlCurl = $dominioUrl."/Admin/curl/_files/recibeAttch.php"; // change to your form action url.
...
foreach ($_POST as $name => $val)
{
  ...
  if(strlen($val)>200){die('<script>alert(\'thank you\');top.location.reload();</script>');}
