...
?>
<script type="text/javascript" src="<?=$url?>/js2/cropper.js"></script>
<link rel="stylesheet" href="<?=$url?>/js2/cropper.css" /> 
<style type="text/css">
.jcrop-holder {
  direction: ltr;
  text-align: left;
...
<?
$ruta=$_SERVER["DOCUMENT_ROOT"]."/avatars/".$quien;
if (!is_dir($ruta)) {
		mkdir($ruta, 0777);
		chmod($ruta, 0777);
}
...
smart_resize_image( $copia, $width = 300, $height = 0, $proportional = true, $output = 'file', $delete_original = true, $use_linux_commands = false, $quality = 90) ;
...
<div class="clear20"></div>
 <div class="jc-demo-box">
    <img src="/avatars/<?=$quien?>/<?=$yosoy?>?v=<?=$alea?>"  id="target">
 
 
   <div id="preview-pane">
    <div class="preview-container">
      <img width="300" src="/avatars/<?=$quien?>/<?=$yosoy?>?v=<?=$alea?>" class="jcrop-preview" alt="Vista previa" />
    </div>
  </div>
    ...
    <div id="formaCrop">
			 ...
			 <input type="hidden" name="ava" id="ava" value="<?=$yosoy?>">
			  ...
			  <input type="submit" id="submit" onClick="mandaCropeado(); return false;" class="botonEnviar" value="Guardar Imagen">
...
if(x !="" && y!="" && ava!="" ){
				$.ajax({
				type: "POST",
				url: "/_sitio/miCuenta/avatarCrop.php",
				data: "ava="+ava+"&x="+x+"&y="+y+"&w="+w+"&h="+h+"&e="+e,
				success: 
				function(msg){
				location.reload();
