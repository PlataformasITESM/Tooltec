...
$res6 = $mysqli->query("SELECT * FROM quesosPlatillos WHERE id='$valor'");
$res6->data_seek(0);
...
<form action="platillosAddForma" method="post" enctype="multipart/form-data" id="forma">
 ...
 <div class="div50">
  ...
  <input type="hidden" id="borraProceso" value="quesosPlatillos" >
...
$res6 = $mysqli->query("SELECT * FROM quesosPlatillos WHERE id='$cambia'");
$res6->data_seek(0);
 while ($fila = $res6->fetch_assoc()) 
	{
...
if($cambia!=""){
	$query="UPDATE quesosPlatillos SET    titulo_es='$titulo_es', tituloC_es='$tituloC_es',  titulo_en='$titulo_en', tituloC_en='$tituloC_en' ,modificado='$creado'   WHERE id='$cambia'";
	$mysqli->query($query); 
}
...
$query="INSERT INTO quesosPlatillos (id,    titulo_es,  tituloC_es, titulo_en,  tituloC_en,       creado) VALUES ('$cambia',     '$titulo_es',  '$tituloC_es',  '$titulo_en',  '$tituloC_en',     '$creado')";
$mysqli->query($query); 
}
...
unlink($rutaServer.'/productos/usos/'.$cambia.'/t_'.$img);
	...
	$query="UPDATE quesosPlatillos SET  img='$yosoy' WHERE id='$cambia'";
	$mysqli->query($query);
}
...
top.location.href = "<?=$url?>/_quesos/platillos" ;</script>	
<?
 } ?>