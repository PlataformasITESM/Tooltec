...
while ($fila = $res6->fetch_assoc()) 
{
	...
	$som=$arregloRegistros[$esteCuando]['totalVistas'] ?? 0;
	$arregloRegistros[$esteCuando]['totalVistas']=$som+1;
	
	$som2=$arregloRegistros[$esteCuando][$idHerramienta]['vistas'] ?? 0;
	$arregloRegistros[$esteCuando][$idHerramienta]['vistas']=$som2+1;
...
//grafi
	...
	foreach ( $period as $dt ) {
	  ...
	  if(!isset($registros)){$registros=0;}
		...
		$visitas=$arregloRegistros[$esteDia]['totalVistas'] ?? 0; if($visitas<1){$visitas=0;}
		...
		$valin=$arregloRegistros[$esteDia][$idH]['vistas'] ?? 0;
		if($valin<1){$valin=0;}
...
var chart = AmCharts.makeChart("chartdiv", {
	...
	$cuenta=1;
	foreach ($arregloHerras as $idH=>$tis){
		$valin=$arregloRegistros[$esteDia][$idH]['vistas'];
		if($valin<1){$valin=0;}
			$esteA['value'.$cuenta]=$valin;
			$cuenta=$cuenta+1;
			?>
