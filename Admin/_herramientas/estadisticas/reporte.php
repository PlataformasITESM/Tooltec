...
$selas="SELECT * FROM herramientasDescargas WHERE     idHerramienta='$valor' ";
$res6 = $mysqli->query($selas);
$res6->data_seek(0);
while ($fila = $res6->fetch_assoc()) 
{
$arregloDes[$fila['id']]=$fila['titulo'];
}
...
if($fechaI=="" && $fechaF==""){$fechaBusca="AND date(cuando)='$hoyBusca'";}
...
$selas="SELECT * FROM herramientasVistas WHERE     idHerramienta='$valor' $fechaBusca   order by cuando asc";
$res6 = $mysqli->query($selas);
$res6->data_seek(0);
while ($fila = $res6->fetch_assoc()) 
{
...
<td style=" text-transform: capitalize;"><?=$tipoUsuario?></td>
<td><?=$nombre?></td>
<td>Vista</td>
</tr>
...
$selas="SELECT * FROM herramientasLikes WHERE     idHerramienta='$valor' $fechaBusca   order by cuando asc";
$res6 = $mysqli->query($selas);
$res6->data_seek(0);
while ($fila = $res6->fetch_assoc()) 
{
	...
	$idUsuario=$fila['idUsuario'];
	$info=unserialize($fila['creado']);
	$arregloRegistros[$esteCuando]['likes']=$arregloRegistros[$esteCuando]['likes']+1;
...
$selas="SELECT * FROM herramientasDes WHERE     idHerramienta='$valor' $fechaBusca   order by cuando asc";
$res6 = $mysqli->query($selas);
$res6->data_seek(0);
while ($fila = $res6->fetch_assoc()) 
{
	$cuando=$fila['cuando'];
	$idDescarga=$fila['idDescarga'];
	$esteCuando=explode(' ',$cuando)[0];
	$idUsuario=$fila['idUsuario'];
	 
	 $arregloRegistros[$esteCuando]['descargas']=$arregloRegistros[$esteCuando]['descargas']+1;
...
<td style=" text-transform: capitalize;"><?=$tipoUsuario?></td>
<td><?=$nombre?></td>
<td>Descarga <?=$arregloDes[$idDescarga]?></td>
</tr>
...
//grafi
	...
	$registrosR=$arregloRegistros[$esteDia]['registrado'];
	$registrosV=$arregloRegistros[$esteDia]['visitante'];
	$registrosL=$arregloRegistros[$esteDia]['likes'];
	$registrosD=$arregloRegistros[$esteDia]['descargas'];
	  ...
	  if($registros==""){$registros=0;}
			...
			if($registrosD==""){$registrosD=0;}
		
	  	$arregloGrafica[]=' { "date": "'.$esteDia.'",  "category": "Visitas",
 "value1": '.$registros.', "value2": '.$registrosR.', "value3": '.$registrosV.', "value4": '.$registrosL.', "value5": '.$registrosD.' }';
...
var chart = AmCharts.makeChart("chartdiv", {
    ...
    {
        "title":"Registrados",
        "balloonText": "Registrados<br><b><span style='font-size:14px;'>[[value]]</span></b>",
        "bullet": "round",
        "dashLength": 2,
        "colorField":"color",
        "valueField": "value2"
	...
	{
        "title":"Visitantes",
        "balloonText": "Visitantes<br><b><span style='font-size:14px;'>[[value]]</span></b>",
        "bullet": "round",
        "dashLength": 2,
        "colorField":"color",
        "valueField": "value3"
	...
	{
        "title":"Likes",
        "balloonText": "Likes<br><b><span style='font-size:14px;'>[[value]]</span></b>",
        "bullet": "round",
        "dashLength": 2,
        "colorField":"color",
        "valueField": "value4"
	...
	{
        "title":"Descargas",
        "balloonText": "Descargas<br><b><span style='font-size:14px;'>[[value]]</span></b>",
        "bullet": "round",
        "dashLength": 2,
        "colorField":"color",
        "valueField": "value5"
