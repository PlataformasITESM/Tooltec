...
var busqueda=settings.busqueda;
        // Apply options
		...
		if(busqueda!==false){
		 apenda='<div class="filtro" data-tipo="'+tipo+'" data-columna="'+columna+'" >';
...
if(hayLiga){
	top.location.href=hayLiga+"&search="+busca;
	}else{
...
// pon las flechitas
 ...
 function reordena(base){ 
	closeAlert();
	 ...
	 $('body').prepend('<div id="alerta" class="alerta" style="max-width:300px; text-align: left;"></div>');
	 $('#alerta').prepend('<div id="alertaBox" class="alertaBox" style="margin: auto; overflow-y: auto; heigth:60%;"><div class="" id="sortaReorden"></div></div>'); 
	 ...
	 $( "tr" ).each(function() {
		 ...
		 '<div class="div100" style=" padding:5px 5px 5px 20px">x'+tits+'</div>'+
		 '</div>';
		  if(tits){
...
$.ajax({
    type: "POST",
    url: "/Admin/_api/procesos/reorden.php",
